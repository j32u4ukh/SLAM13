...
int main(int argc, char **argv) {
  ...
  // 沿Z軸轉90度的旋轉矩陣
    ...
    // 李代數se(3) 是一個六維向量，方便起見先 typedef 一下
    ...
    // 李代數 se(3) 使用指數映射獲得它的李群 SE(3)
    Sophus::SE3d SE3_recover = Sophus::SE3d::exp(se3);
    cout << "SE3 recover:\n" << SE3_recover.matrix() << endl;

    Sophus::SE3d rmse_matrix = SE3_recover.inverse() * SE3_recover;
    Vector6d rmse_vector = rmse_matrix.log();
    double rmse = rmse_vector.norm();
    cout << "rmse_matrix = " << rmse_matrix.matrix() << endl;
    cout << "rmse_vector = " << rmse_vector.transpose() << endl;
    cout << "rmse = " << rmse << endl;
