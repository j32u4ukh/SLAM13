...
int main(int argc, char **argv) {
    ...
    for (DMatch m:matches) {
        ...
        // 將深度除以 5000 來控制縮放尺度，可令場景規模控制在一定程度的大小，也令計算時數值更為穩定
        float dd = d / 5000.0;
    ...
    // cv::solvePnP
    // 調用OpenCV 的 PnP 求解，可選擇EPNP，DLS等方法
    solvePnP(pts_3d, pts_2d, K, Mat(), r, t, false); 

    // r為旋轉向量形式，用Rodrigues公式轉換為矩陣
    cv::Rodrigues(r, R);
  ...
  Sophus::SE3d &pose) {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int i = 0; i < points_3d.size(); i++) {
            // 成像平面
            // pose: 第一次進入迴圈時為初始值，後面才會根據誤差去更新 pose
            ...
            // points_2d：實際的投影位置；proj：根據估計的相機位姿計算投影位置
...
public:
    ...
    virtual void linearizeOplus() override {
        ...
        // 微小擾動 對 誤差 的導數
        // JacobianXiOplusType _jacobianOplusXi：繼承自 BaseUnaryEdge 的變數
        _jacobianOplusXi
