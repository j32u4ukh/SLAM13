...
// Descriptor type: 32 bit unsigned int, will have 8, 8x32=256
 ...
 *
 * NOTE: if a keypoint goes outside the image boundary (8 pixels), descriptors will not be computed 
 * and will be left as empty
 */
...
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  ...
  for (auto &kp: keypoints) {
    ...
    // m_sqrt: length of geometry center to centroid
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            for (int k = 0; k < 8; k++) {
              ...
              計算兩個描述子數值不同的個數*/
              // _mm_popcnt_u32：將 32 位二進位數值加總（EX: 1010 -> 4; 0011 -> 2）
